...
public interface QueryDefinition extends Subquery {
	/**
	 * Add a query root corresponding to the given entity, forming
	 * a cartesian product with any existing roots.
	 * The domain object that is returned is bound as a component of
	 * the given query.
	 * The argument must be an entity class.
	 * @param cls - an entity class
	 * @return DomainObject corresponding to the specified
	 * entity class.
	 */
	DomainObject addRoot(Class cls);
	/** Add a root derived from a domain object of the containing
	 * query definition to a query definition used as a subquery.
	 ...
	 * Replaces the previous select list, if any.
	 * If no select items are specified and there is only one
	 * query root, the root entity is assumed to be the result.
	 * @param selectItems - one or more SelectItem instances
	 ...
	 * If no select items are specified and there is only one
	 * query root, the root entity is assumed to be the result.
	 * @param selectItemList - a list containing one or more
	...
	/**
	 * Specify the objects / values to be returned.
	 * Duplicate results will be eliminated.
	 * Replaces the previous select list, if any.
	 * If no select items are specified and there is only one
	 * query root, the root entity is assumed to be the result.
	 * @param selectItems - one or more SelectItem instances
	...
	/**
	 * Modifies the query definition to restrict the
	 * result of the query according to the specified predicate.
	 * Replaces the previously added restriction(s), if any.
	 * @param predicate - a simple or compound conditional predicate
	 * @return the modified QueryDefinition instance
	 */
	QueryDefinition where(Predicate predicate);
	/**
	 * Specify the items of the select list that are used
	 * in ordering the query results.
	 * Replaces the previous order-by list, if any.
	 * @param orderByItems - one or more OrderByItem instances
	 * @return the modified QueryDefinition instance
	 */
...
Sun Microsystems, Inc.
	...
	/**
	 * Specify that a constructor for the given class is to
	 * be applied to the corresponding query results after
	 * the query is executed.
	 * The class must have a constructor that accepts the Java
	 * argument types corresponding to the given select items.
	 * @param cls - a class with the correponding constructor
	 * @param args - select items that correspond to result
	 * types that are valid as arguments to
	 * the constructor
	 * @result SelectItem instance representing the constructor
	 */
	SelectItem newInstance(Class cls, SelectItem... args);
	/**
	 * Use the query definition instance as a subquery in an
	 * exists predicate.
	 * @return the resulting predicate
	 */
	Predicate exists();
	/**
	 * Use the query definition object in a subquery in
	 * an all expression.
	 ...
	 * A general case expression is of the form:
	 *
	 * generalCase()
	...
	/**
	 * Create a simple case expression with
	 * the given case operand.
	 * A simple case expression is of the form:
	 *
	 * simpleCase(case-operand)
	 * .when(scalar-expression).then(scalar-expression)
	 * .when(scalar-expression).then(scalar-expression)
	 * ...
	 * .elseCase(scalar-expression)
	 *
	 * @param caseOperand - expression used for testing
	 * against the when scalar expressions
	 ...
	 * .elseCase(scalar-expression)
	 *
	 * @param caseOperand - numeric value used for testing
	 * against the when scalar expressions
	 * @return case expression with the given case operand
	 */
	 ...
	 * .elseCase(scalar-expression)
	 *
	 * @param caseOperand - value used for testing against
	 * the when scalar expressions
	 * @return case expression with the given case operand
	 */
	...
	/**
	 * coalesce
	 * This is equivalent to a case expression that returns
	 * null if all its arguments evaluate to null, and the
	 * value of its first non-null argument otherwise.
	 * @param exp - expressions to be used for testing
	 * against null
	 * @return Expression corresponding to the given
	 * coalesce expression
	 */
	...
	/**
	 * nullif
	 * This is equivalent to a case expression that tests
	 * whether its arguments are equal, returning null
	 * if they are and the value of the first expression
	 * if they are not.
	 * @param exp1
	 * @param exp2
	 * @return Expression corresponding to the given
	 ...
	 * @param arg1
	 * @param arg2
Selector API Java Persistence 2.0, Public Review Draft Selector API Interfaces
10/31/08 158 JSR-317 Public Review Draft
Sun Microsystems, Inc.
	 * @return Expression corresponding to the given
	 * nullif expression
	 */
	...
	/**
	 * Create an Expression corresponding to the current time
	 * on the database server at the time of query execution.
	 * @return the corresponding Expression
	 */
	Expression currentTime();
	/**
	 * Create an Expression corresponding to the current date
	 * on the database server at the time of query execution.
	 * @return the corresponding Expression
	 */
	Expression currentDate();
	/**
	 * Create an Expression corresponding to the current timestamp
	 * on the database server at the time of query execution.
	 * @return the corresponding Expression
	 */
